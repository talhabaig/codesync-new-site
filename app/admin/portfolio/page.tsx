...
import { useGetPortfolios } from "../../../features/portfolio/hooks/useGetPortfolios";
import { usePortfolioMutations } from "../../../features/portfolio/hooks/usePortfolioMutations";

const COVER_FALLBACK = "/icon.png";

const blankPortfolio = (): CreatePortfolioPayload => ({
  title: "",
  shortDescription: "",
  coverImage: "",
  siteUrl: null,
...
export default function AdminPortfolio() {
  const [params, setParams] = useState<GetPortfoliosParams>({
    page: 1,
    limit: 5,
    sortBy: "displayOrder",
    sortOrder: "asc",
  ...
  const handleStatusFilter = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setParams((prev) => ({
      ...prev,
      status: val === "ALL" ? undefined : (val as PortfolioStatus),
      page: 1,
    }));
  };
  ...
  const confirmPendingDelete = async () => {
    ...
    try {
      if (pendingDelete.ids.length === 1) {
        await deletePortfolio(pendingDelete.ids[0]);
  ...
  const allOnPageSelected =
    portfolios.length > 0 && portfolios.every((item) => selected.includes(item.id));
    ...
    <div className="">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-gray-900">Portfolio</h1>
          <p className="mt-1 text-sm text-gray-500">
            Manage case studies displayed on your website.
          </p>
        </div>
        <CustomButton
          type="button"
          onClick={() => {
            setEditing(blankPortfolio());
            setEditingId(null);
          }}
          className="bg-customNavy hover:bg-customNavy/90"
        >
          <FaPlus className="h-3.5 w-3.5" /> Add portfolio
        </CustomButton>
      </div>
      ...
      <section className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        ...
        {selected.length > 0 && (
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-blue-100 bg-blue-50 px-4 py-3 text-sm">
            <span className="font-medium text-blue-900">
              {selected.length} portfolio{selected.length === 1 ? "" : "s"} selected
            </span>
            <CustomButton
              type="button"
              variant="danger"
              ...
              onClick={() =>
                setPendingDelete({
                  ids: selected,
                  label: `${selected.length} selected portfolio${selected.length === 1 ? "" : "s"}`,
                })
              }
          ...
          <table className="w-full table-fixed text-left text-sm">
            ...
            <thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
              <tr>
                ...
                <th className="px-3 py-3.5 font-semibold">Project</th>
            ...
            <tbody className={`divide-y divide-gray-100 ${isRefetching ? "opacity-60" : ""}`}>
              {isFetching && portfolios.length === 0 ? (
                <tr>
        ...
        <div className="flex flex-col gap-3 border-t border-gray-200 px-4 py-3.5 text-sm text-gray-600 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            ...
            <span>of {meta.total} portfolios</span>
          </div>
        ...
        >
          <PortfolioForm
            ...
            onSubmit={async (values) => {
              if (editingId) await updatePortfolio(editingId, values);
              else await createPortfolio(values);
              setEditing(null);
              setEditingId(null);
            }}
          />
